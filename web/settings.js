(() => {
  ...
  const enabled = $("stt-enabled"), backend = $("stt-backend"), model = $("stt-model"), download = $("stt-download");
  let current = null;
  ...
  form.addEventListener("submit", async (event) => { event.preventDefault(); note.textContent = "Saving…"; const stt = { ...(current?.stt || {}), enabled: enabled.checked, backend: backend.value, model: model.value, auto_download: download.checked }; try { const result = await request("/api/settings", { method: "POST", body: JSON.stringify({ stt }) }); note.textContent = result.restart_required ? "Saved. Restart GrepMesh to apply." : "Saved."; await load(); } catch (error) { note.textContent = `Save failed: ${error.message}`; } });
  load();
})();
