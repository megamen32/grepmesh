(() => {
  ...
  async function loadHostStatus(){if(state.statusPending)return;state.statusPending=true;try{const data=dataOf(await request("/api/host-status"));state.statuses=Array.isArray(data.nodes) ? data.nodes : [];state.statusStale=false;}catch{state.statusStale=true;}finally{state.statusPending=false;renderHosts();renderHostDetail();}}
